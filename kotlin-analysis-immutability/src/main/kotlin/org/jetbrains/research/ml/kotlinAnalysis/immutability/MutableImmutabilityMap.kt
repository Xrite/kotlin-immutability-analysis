...
/**
 * Class that is used in [solve] to calculate immutability properties of given entities.
 * Allows getting immutability properties of entities provided in constructor and using provided assumptions.
 * Allows capturing entity using [WithContext].
 * @see Immutability
 * */
class MutableImmutabilityMap(private val entities: List<Entity>, private vararg val assumptions: Assumptions) :
    ...
    operator fun set(descriptor: DeclarationDescriptor, newValue: ImmutabilityProperty): Boolean {
        ...
        // TODO: think about replacing `hasSameStatus` to `equals` or smth
    ...
    /**
     * Capture entity and create [ImmutabilityWithContext].
     * @see ImmutabilityWithContext
     */
    inner class WithContext(
        private val descriptor: DeclarationDescriptor,
        private val contextParameters: List<TypeParameterDescriptor>
    ) : Immutability by this,
        ...
        override fun resolveType(
            ...
            when (it) {
                        ...
                        when {
                            ...
                            parameters[i].type.isTypeParameter() -> {
                                ...
                                if (idx != null) {
                                    ...
                                    throw IllegalArgumentException("Can't get index of ${parameters[i]} in ${descriptor.defaultType} with context $contextParameters")
                                }
                            }
                            else -> {
                ...
                is ImmutabilityProperty.Immutable -> {
                    val reason = if (it.isByAssumption())
                        ImmutabilityWithContext.Result.Immutable.Reason.ASSUMPTION
                    else
                        ImmutabilityWithContext.Result.Immutable.Reason.RESOLVED
                    ImmutabilityWithContext.Result.Immutable(reason)
                }
                is ImmutabilityProperty.Mutable -> {
                    val reason = if (it.isByAssumption())
                        ImmutabilityWithContext.Result.Mutable.Reason.ASSUMPTION
                    else
                        ImmutabilityWithContext.Result.Mutable.Reason.RESOLVED
                    ImmutabilityWithContext.Result.Mutable(reason)
                }
                is ImmutabilityProperty.ShallowImmutable -> {
                    val reason = if (it.isByAssumption()) {
                        ImmutabilityWithContext.Result.ShallowImmutable.Reason.ASSUMPTION
                    } else {
                        ImmutabilityWithContext.Result.ShallowImmutable.Reason.RESOLVED
                    }
                    ImmutabilityWithContext.Result.ShallowImmutable(reason)
                }
            }
